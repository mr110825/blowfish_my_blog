...
**定義**: 基数変換問題で無限小数を判定するような問題では**分母の素因数分解**で判定できる。
...
**定義**: 個々の要素の位置を固定して、要素が格納されている番地(アドレス)を簡単に計算できるようにしたデータ構造
...
**定義**: 各要素をポインタでつないだデータ構造

**分類**: ポインタの接続方法で3種類に分類できる

### 単方向リスト

**定義**: 各要素は次の要素へのポインタを持つ

**別名**: 線形リスト・片方向リンク

### 双方向リスト

**定義**: 各要素は前後両方の要素のポインタを持つ

**別名**: 双方向リンク

**単方向リストとの違い**: 途中要素への挿入・削除は双方向が容易

### 循環リスト

**定義**: 末尾の用が先頭要素へのポインタを持つ単方向リスト
...
**定義**: データを最小単位(要素が1つ)になるまで半分に分割し、その後、それらを正しい順序で「マージ(併合)」していくことで全体のソートを完成させるソート方法
**出題問題**: H26秋午前問06

### 値呼び出し

**定義**: 引数の渡し方において、関数内で変数の値を上書きしても元の変数の値には反映されない。
**出題問題**: H28春午前問20

### 参照呼出し

**定義**: 引数の渡し方において、関数内で変数の値を上書きすると元の変数の値に反映される。
**出題問題**: H28春午前問20

### 再入可能プログラム

**別名**: リエントラントプログラム
**定義**: 複数のタスクから同時に呼び出されても、それぞれに対して正しい結果を返すことができるプログラム
**再入可能プログラムの実現方法**:
    - プログラムを「手続部分」と「データ部分」に分離する
    - 複数のタスクで共有するのは「手続部分」・「データ部分」は各タスク単位で用意することで再入可能プログラムを実現できる。

### 再帰プログラム

**別名**: リカーシブプログラム
**定義**: 手続きの中で自分自身を呼び出して使うことができるプログラム
**特徴**:
    - 自分自身を呼び出すことができる
...
**別名**:
    - リユーザブルプログラム
    - 逐次再使用可能プログラム
**定義**: 一度実行したプログラムをロードし直さずに再度実行しても、正しい結果を返すことができるプログラム

### 再配置可能プログラム

**別名**: リロケータブルプログラム
**定義**: プログラムを主記憶上のどのアドレスに配置しても実行できるようにしたプログラム
**特徴**:
...
**正式名称**: Very Long Instruction Word
**定義**: プログラムのコンパイル時に、一つの命令語で複数の命令を同時に実行する手法。
**特徴**:
...
**定義**: メモリ領域を「ページ」と呼ばれる固定サイズの区画に分割して仮想記憶を管理する方式
**補足**: 仮想アドレス空間と物理アドレス空間をページ単位で対応付けて管理する
**出題問題**: H19春午前問24、R03春午前問19
  - ページ置換の基本的な流れ
    - 1. ページフォールトが発生
...
**定義**: 主記憶や補助記憶装置にて使用されない記憶領域の断片が多く存在した状態になる現象
**出題問題**: H19春午前問24

### スラッシング

**定義**: メモリ容量が不足している状態で、要求されるメモリ量が多すぎるために、メモリと補助記憶装置間でのデータの入れ替え(ページイン・ページアウト)が非常に頻繁に発生し、システムの処理速度が著しく低下してしまう現象
**発生するケース**: 主記憶の容量が十分でない場合、プログラムの多重度を増加させると、システムのオーバーヘッドが増加するため、発生しやすい。
**出題問題**: H19春午前問24

### キャッシュメモリにおけるダイレクトマップ方式

**定義**: 一つのメモリブロックをキャッシュ内の単一のロケーションに割り当てる。
...
**定義**: 「オブジェクト」という単位でデータを格納・管理する記憶方式
**具体例**: Amazon S3
**出題問題**: R06春午前問13
...
**定義**: システムを3層(プレゼンテーション層・ファンクション層・データベースアクセス層)に分けたシステム
    - プレゼンテーション層:
...
**定義**: PCから大型コンピュータまで，ネットワーク上にある複数のプロセッサに処理を分散して，大規模な一つの処理を行う方式

### CAP定理

**定義**: 分散処理システムにおいては、一貫性・可用性・分断耐性の3つの特性のうち、最大でも同時に2つまでしか満たすことができないとする定理
...
**定義**: 故障してから次に故障するまでの平均時間

### MTTR(Mean Time To Repair / 平均修理時間)

**定義**: 故障時に修理が完了するまでの平均時間
...
**定義**: 故障してもシステム全体として必要な機能を維持させようとする考え方

### フォールトトレラントシステム

**定義**: フォールトトレランスの考えが反映されたシステム

### フォールトアポイダンス

**定義**: 構成要素個々の品質を高めて、故障そのものの発生を防ぐこと

### フェールソフト

**定義**: 故障発生時に、性能の低下を受け入れて、システム全体を停止させずに稼働を維持させる考え方

### フォールバック(縮退運転)

**定義**: フェールソフトにおいて、機能低下状態で稼働をつづけること

### フェールセーフ

**定義**: 誤作動や故障時の影響を最小限にとどめて、システムを安全に制御するという考え方
**具体例**: 信号機の故障時にすべての信号を赤信号の状態にする。

### フェールオーバ

**定義**: 障害発生時に別システムに自動的に引き継がせて利用者に故障などを悟らせないという考え方
  - 障害回復時に元のシステムへ戻すことを「フェールバック」と呼ぶ。

### フォールトマスキング

**定義**: 障害が発生しても、影響が外部に出ないようにする考え方

### フールプルーフ

**定義**: 誤った操作などされても、システムに異常が出ないように設計する考え方
...
**定義**: OSとしてコンピュータやアプリを制御するための基本的な機能のみを実装したソフトウェア
**より詳細な定義**: カーネルはOSの中核(コア)で、主にハードウェア(CPU、メモリ、デバイス)とアプリケーション(プロセス)の橋渡し役を担い、重要なリソース管理やプロセス管理などの基本機能を持つソフトウェア
**「基本的な機能」の補足**: 基本的な機能とは、プロセス管理、メモリ管理、デバイス管理、ファイルシステム管理、ネットワーク管理などを指す

### タスクのスケジューリング方式

**定義**: 複数のタスクを並列処理する場合の処理方法
**分類**:
...
**定義**: 一度CPUの使用権を得たタスクが、自らの処理を終えるか、I/O待ちなどで自発的にCPUを解放(手放す)するまで、他のタスクにCPUを横取りされない方式
**特徴**:
    - OSがタスクを強制的に中断させない
...
**定義**: OSのスケジューラが、より優先度の高いタスクが来た場合や、一定の時間が経過した場合などに、現在実行中のタスクを強制的に中断させ、別のタスクにCPUの使用権を割り当てる方式
**特徴**:
    - OSのスケジューラがタスクの切り替えを実施する
      - このとき中断されたタスクが再開されるとき中断された状態から再開される
**別名**: 強制的(プリエンプティブ)マルチタスク

### 到着順番方式

**定義**: タスクには優先度が設定されず、実行可能状態になったタスクから処理される方式
**別名**: FCFS(First Come First Served)

### 優先順位方式

**定義**: タスクに割り当てられた優先度の高い順に実行する方式
**優先順位方式の種類**:
...
**定義**: タスクにCPU使用権が割り当てられず、なかなか実行されてない状態
  - リソーススタベーション
    - タスクの実行に必要なリソースを与えられない状態

### ラウンドロビン方式

**定義**: 実行可能待ち行例の先頭のタスクから順番にCPU時間(タイムクウォンタム)を割り当てる方式
    - 補足:
...
**定義**: ラウンドロビン方式に優先度を加えて方式
**別名**: 多段待ち行列方式

### 処理時間順方式

**定義**: 処理時間の短いタスクから順番に実行する方式
**別名**: SPT(Shortest Proccessing Time First)方式
**補足**: 実際には残り処理時間を予測することは不可能のため、フィードバック待ち行列方式として実現されている。

### Hadoop

**定義**: ペタバイト級の大規模データの蓄積・処理の分散処理を実現するミドルウェア
**補足**: 主に「HDFS(Hadoop Distributed File System)」という分散ファイルシステムと「MapReduce」という分散処理プログラミングモデルを中心に構成されている

### GPL

**正式名称**: General Public License
**定義**: GNUプロジェクトのためにリチャード・ストールマンにより制作されたフリーソフトウェアライセンス
**特徴**: 「コピーレフト」という概念を採用している
    - コピーレフト:GPLでライセンスされたソフトウェアを改変またはそれを基にした派生著作物を配布する際に、そのソフトウェアや派生著作物もGPLのもとで配布しなければならないというルール
**出題問題**: H25春午前問21

### MOSトランジスタ

**定義**: 金属と半導体の間に酸化物絶縁体を挟んだ構造を持つ半導体素子。金属(M)、酸化膜(O)、半導体(S)の三層構造を持つ。
...
**定義**: 必要とされる全ての機能(システム)を集積した1個の半導体チップ
**メリット**:
    - 専有面積の削減
    - 高速化
    - 低消費電力化
...
**定義**: デジタル回路の設計や構成をテキストベースで記述できるハードウェア記述言語の総称
**具体例**: VHDL、Verilog、SystemCなど
**使用用途**: FPGAなどに実装するデジタル回路を記述し，論理合成するために使用される
**出題問題**: R07春午前問20

### DSP

**正式名称**: Digital Signal Processor
**定義**: 主にリアルタイムコンピュータで使用される、デジタル信号処理に特化したプロセッサ
**特徴**: 積和演算などの機能を内蔵しているので，デジタルフィルターを実現するのに適している。
...
**定義**: スクリーンの走査線ごとに視点とその走査線を結ぶ走査面を作成し，各走査面と描画の対象となる物体との交差を調べて交差線分を求め，奥行き判定を行うことによって描画する手法
  - レイトレーシング法との違い
...
**定義**: 全てのノードがリング(輪)のように一方向または双方向に接続される方式
**補足**: LANの代表的な接続形態(トポロジー)の一つ
  - 図解
    ```mermaid
    graph LR
      %% リング型トポロジー
...
**定義**: 中心となるネットワーク機器(スイッチやハブ)を中心に、各機器が放射状に接続される形態
**補足**: LANの代表的な接続形態(トポロジー)の一つ
  - 図解
    ```mermaid
    graph LR
...
**定義**: すべてのデバイスが一本の主幹ケーブル(バス)に接続され、データはこのバスを通じて送受信される形式
**補足**: LANの代表的な接続形態(トポロジー)の一つ
  - 図解
    ```mermaid
    graph LR
...
**定義**: 通信前に受信相手を確認せずに一方的にデータを送信する方式。
**特徴**: 送信するパケットに宛先を示す情報を付与する必要がある。
**具体例**: パケット交換方式
**出題問題**: H17秋午前問53
...
**定義**: 送信するデータが正確に、かつ送信した順序通りに相手に届くことを保証する、信頼性の高い通信プロトコル
**特徴**:
    - OSI参照モデルの第4_トランスポート層に位置する。
...
**正式名称**: Multipurpose Internet Mail Extension
**定義**: メールでにて日本語の"2バイトコード"や画像データを送信するための仕組み
**使用用途**: ASCII文字しか使用できないSMTPを利用したメールで利用される。
**出題問題**: H18春午前問52

### IMAPS

**正式名称**: IMAP over SSL/TLS
**定義**: メール受信プロトコルであるIMAP(Internet Message Access Protocol)にTLSを組み合わせ、TLSによって暗号化された通信コネクション上でメール受信を行うプロトコル
**補足**: バージョン番号を付けてIMAP4Sと表記されることもある。

### ポート番号

**定義**: ネットワーク上でデータ通信を行う際に、コンピュータ内で動作している特定のプログラムやサービスを識別するための番号
**URLで宛先のポート番号を指定する場合**: ホスト名の直後に":"(コロン)と数字で指定する
  ![ポート番号の指定方法](../../../90_Attachments/img/port_number_specification.png)

### ウェルノウンポート

**定義**: インターネットプロトコルにおいて、特定のサービスやアプリケーション用に予約された標準的なポート番号(0-1023番)
**特徴**:
...
**主要なウェルノウンポート一覧**:
    ...
    - DNS - ポート53/TCP・UDP
      - 用途:ドメイン名前解決
    - DHCP(サーバ用) - ポート67/UDP
      - 用途:IPアドレス自動割り当て(サーバ側)
    - DHCP(クライアント用) - ポート68/UDP
      - 用途:IPアドレス自動割り当て(クライアント側)
    - HTTP - ポート80/TCP
      - 用途:Web通信(平文)
    - POP3 - ポート110/TCP
      - 用途:メール受信
    ...
    - SNMP - ポート161/UDP
      - 用途:ネットワーク機器管理・監視
    - SNMP Trap - ポート162/UDP
      - 用途:ネットワーク機器からの通知・アラート
    - HTTPS - ポート443/TCP
      - 用途:暗号化Web通信(SSL/TLS)

### DNSレコード

**定義**: ドメイン名とIPアドレスを紐づけるなど、インターネット上でドメイン名を管理するための重要な情報

### Aレコード (Address Record)

**定義**: ドメイン名(ホスト名)をIPv4アドレスに対応付ける、最も基本的なDNSレコード
**用途**: ユーザーがブラウザにウェブサイトのドメイン名を入力した際に、そのサイトが置かれているサーバーのIPアドレスを特定

### AAAAレコード (Quad A Record)

**定義**: AレコードのIPv6版で、ドメイン名(ホスト名)をIPv6アドレスに対応付ける
**用途**: IPv6アドレスでウェブサイトにアクセスするために必要

### CNAMEレコード (Canonical Name Record)

**定義**: あるドメイン名(エイリアス)を別の正規のドメイン名(Canonical Name)に関連付けるために使用
**用途**: 複数のドメイン名を一つのウェブサイトに向けたり、管理を簡素化する
**特徴**: IPアドレスを直接指定するのではなく、別のドメイン名を指し示す

### MXレコード (Mail Exchange Record)

**定義**: そのドメイン宛てのメールを配送するメールサーバーを指定するためのレコード
**用途**: メールサーバーがどのサーバーにメールを届ければよいかを確認
**特徴**: 優先度の数値が小さいサーバーから順に配送が試行される

### PTRレコード (Pointer Record)

**定義**: IPアドレスからドメイン名(ホスト名)を特定するために使用される(逆引き)
**用途**: メールサーバーが受信したメールの送信元IPアドレスの信頼性を確認
**特徴**: なりすましメール対策として利用される。設定が正しくないとメールがスパムとして扱われる可能性がある

### OP25B(Outbound Port 25 Blocking)

**定義**: ISPが用意したスパムメール対策の仕組み
**補足**: ISPが用意したメールサーバーを経由しないメール(高確率でスパムメール)を弾くようになっている。
**OP25Bの対象外になる対象**:
    - 固定IPからの送信
...
**定義**: メールを送信しようとしてきたメールサーバのIPアドレス情報を検証することで身元検証する技術
  - SPFの手順
    ...
    ![alt text](../../90_Attachments/img/image.png_1759242749795)
...
**定義**: 無線LAN(Wi-Fi)の技術規格
**特徴**:
    - 802.11a:5GHz帯、最大54Mbps
    - 802.11n:2.4GHz/5GHz両対応、MIMO技術でより高速化
**用途**: 無線ネットワークアクセス、Wi-Fi接続
**注意**: 認証技術ではない

### IEEE 802.1Q

**定義**: タグVLAN(仮想LAN)の技術規格
...
**定義**: DNSサーバに名前解決を問い合わせ、結果を確認するコマンド
...
**定義**: 宛先への到達可否と遅延(往復時間)を確認するコマンド
**用途**: 基本的な疎通確認

### tracert

**定義**: 宛先までに経由するルータの一覧と遅延を可視化するコマンド
**用途**: ネットワーク経路の調査

### arp

**定義**: IPアドレスとMACアドレスの対応表を表示・更新するコマンド
**用途**: ARPテーブルの確認・操作

### ipconfig

**定義**: 端末のネットワーク設定の表示・更新するコマンド
**用途**: ネットワーク設定の確認・変更
...
**定義**: 情報セキュリティマネジメントの付加的な要素で、行った操作や発生した事象を後になって否認されないように証明できる能力
**補足**: JIS Q 27000:2019(情報セキュリティマネジメントシステム－用語)において定義されている

### CC

**正式名称**: Common Criteria
**別称**: 情報セキュリティ国際評価基準
**定義**: IT製品やシステムのセキュリティ機能を評価し認証するための基準を定めた規格
**補足**: ほぼそのままの形でISO 15408(日本版では JIS X 5070)として国際標準化されている。

### 耐タンパ性

**定義**: 暗号システムやハードウェアが物理的な不正操作や改ざん(タンパー)に対して耐えられる能力
...
**定義**: 対象とする範囲でセキュリティ上の問題が起きていないかどうかを監視するとともに、発生したセキュリティインシデントについて対応するチームや組織の総称

### PSIRT

**定義**: 製品セキュリティインシデント対応チーム
**言い換えると**: 自社製品版のCSIRT
...
**別称**: MITB
**定義**: PCに侵入したマルウェアが，利用者のインターネットバンキングへのログインを検知して，Webブラウザから送信される振込先などのデータを改ざんする攻撃

### クリプトジャッキング

**定義**: マイニング作業を他のコンピュータに秘密裏に代行させる行為
    - マイニング:暗号資産を入手するための計算処理
**出題問題**: R02秋午前問41
...
**定義**: 本来は検知すべき悪意のある活動を、誤って害のないものとして分類すること。
    - 自分なりに言い換えると「セキュリティが対象を見逃すこと」

### フォールスポジティブ

**定義**: 本来は通過させるべき害のない活動を、誤って悪意のあるものとして分類すること。
...
**定義**: 一度のログインで複数のクラウドサービスやアプリケーションにアクセス可能にする「シングルサインオン(SSO)」を実現するための標準規格
...
**SAML認証の仕組み**:
  ...
  - OAuthとの違い
    - SAMLとよく比較される規格に「OAuth(オーオース)」があります。両者の主な違いは役割です。
      - SAML:ユーザーが誰であるかを証明する「認証」と、そのユーザーに何をする権限があるかを示す「認可」の両方を扱います。
      - OAuth:主に「認可」を目的としたプロトコルです。例えば、「あるアプリにGoogleアカウントの写真へのアクセスを許可する」といった場面で使われます。ユーザー自身の認証は行いません。

### 3Dセキュア

**定義**: オンライン決済時に本人のみが知る情報(パスワードや属性情報等)を入力させることで、利用者本人が取引を行っていることを確認する仕組み
**補足**: 事前にカード発行会社に登録したパスワードやワンタイムパスワード、SMS認証、生体認証などで本人確認を行い、不正利用を防ぐ
**名称の由来**: なんで3Dセキュアって呼ぶの?
...
**定義**: 無線LANのセキュリティを保護するための規格
**利用される暗号アルゴリズム**: AES
    - なぜ、RSAやECCではない?:
      - 通信データの暗号化には高速な「AES」が適しており、RSAやECCは処理が遅いため

### CRL

**正式名称**: Certificate Revocation List
**別称**: 証明書失効リスト
**定義**: 公開鍵基盤(PKI)において失効した(信用性のない)公開鍵証明書のリスト
...
**正式名称**: Petri Net
**定義**: プレース、トランジション、トークンの3つの要素を使用した有向グラフでシステムの動作を記述する図法
...
**定義**: リアルタイムでデジタル証明書の失効情報を検証し、有効性を確認するプロトコル
**OCSPの確認手順**: OCSPクライアントは、確認対象となるデジタル証明書のシリアル番号等をOCSPレスポンダに送信し、有効性検証の結果を受け取ります。この仕組みを利用することで、クライアント自身がCRL(証明書失効リスト)を取得・検証する手間を省くことができます。
![alt text](../../90_Attachments/img/image.png_1759277252888)

### DNSキャッシュポイズニング攻撃

**定義**: DNSサーバーの「キャッシュ」に偽の情報を意図的に注入(ポイズニング＝毒を盛る)することで、ユーザーを正規のウェブサイトになりすました悪意のある偽サイトへ誘導するサイバー攻撃
**DNSの基本的な仕組み**: この攻撃を理解するために、まずDNSの通常の動きを簡単に説明します。
...
**DNSキャッシュポイズニング攻撃**: DNSキャッシュポイズニング攻撃は、上記のステップの**2と3の間**に割り込みます。
...
**DNSキャッシュポイズニング攻撃への対策**: DNSキャッシュポイズニングへの対策は、DNSサーバー管理者側と利用者側の両方で重要です。
    - サーバー管理者側の対策
      - DNSソフトウェアの最新化:既知の脆弱性をなくすため、DNSサーバーのソフトウェアを常に最新の状態に保ちます。
      - ソースポートランダマイゼーション:DNSの問い合わせに使う送信元ポート番号をランダム化し、攻撃者が偽の応答を送り込む際の推測を困難にします。
      - DNSSEC (DNS Security Extensions) の導入:DNSの応答に電子署名を付与し、その応答が正当なサーバーから送られ、改ざんされていないことを検証する仕組みです。最も効果的な対策の一つとされています。
    - 利用者側の対策
...
**正式名称**: DNS Security Extensions
**定義**: DNSの応答が「正当な管理者から発信され、途中で改ざんされていないこと」を保証するための拡張機能
...
**定義**: ユーザーが入力したデータや外部から受け取ったデータに含まれる、システムにとって危険な文字列や記号を、無害なものに変換または除去する処理

### ISMAP

**別称**: 政府情報システムのためのセキュリティ評価制度
**定義**: 国際標準等を踏まえて政府が策定したセキュリティ基準に基づき、各基準が適切に実施されているかを第三者が監査するプロセスを経て、クラウドサービスを登録する国の制度

### IDS
**正式名称**: Intrusion Detection System
**定義**: ネットワークやホストをリアルタイムで監視し、異常を検知した場合に管理者に通知するなどの処置を行うシステム
  - IDSの種類
...
**定義**: ある製品に含まれるソフトウェアに含まれるすべてのコンポーネントについて、それらの名称、バージョン・ビルド情報、ライセンス情報、依存関係、その他関連情報を含めて、機械処理可能なリストにしたもの
...
**定義**: 個別に発見されたコンピューターのソフトウェアやハードウェアの脆弱性(セキュリティ上の欠陥や弱点)に、それぞれ固有の識別番号を割り振るための世界共通の仕組み

**簡単に言い換えると**: 脆弱性につけられる、ユニークなID番号

### TPM
**正式名称**: Trusted Platform Module
**定義**: PCなどの機器に搭載され鍵生成・ハッシュ演算及び暗号処理を行うセキュリティチップ
...
**正式名称**: Systems of Engagement

**定義**: 企業システムのうち、顧客や企業との関係性を深めるために構築されるシステム

**使用用途**: SNSやモバイルアプリ、Webサイトなど。ユーザーとの接点となり、人と人との繋がりを生み、それを維持していくことを主眼とするシステム
...
**定義**: 自己資本に対してどれだけ効率的に利益を上げたかを示す指標
...
\[
ROE = \left(\frac{当期純利益}{売上高}\right) \times \left(\frac{売上高}{総資産}\right) \times \left(\frac{総資産}{自己資本}\right)
= \frac{当期純利益}{自己資本}
\]

### SECIモデル

**定義**: 「共同化→表出化→連結化→内面化」という4つのプロセスを経ることで組織の共有の知識となることを示している
...
**定義**: 従業員満足度・サービス・顧客満足度・利益の因果関係を表したモデル
...
**定義**: 企業活動を「5つの主活動」と「4つの支援活動」に区分して、分析するフレームワーク

**詳細定義**: 企業の活動を主活動(購買、製造、出荷、販売、サービスなど)と支援活動に分類し、価値(マージン)がどの活動で生み出されるかを分析するフレームワーク
...
**マージンの概念**: 総価値と総コストの差がマージン(利益)となる。どの活動で価値が生み出されるかを特定することが重要

**頻出ポイント**: 主活動と支援活動の分類は頻出テーマ、競争優位性を持つ活動の特定、ERP導入時の影響分析(標準化による強みの喪失リスク)

### パーミッションマーケティング

**定義**: 同意を得た顧客に対して直接的にパーソナルな売り込みを行う手法

**出題問題**: R05秋午前問68

### バイラルマーケティング

**定義**: 口コミやインターネットでの情報共有を促進する仕組みを構築して、情報を自然発生的に周知することで認知度を向上させる手法

### 多角化戦略の類型

**定義**: アンゾフの成長戦略における多角化戦略の類型
...
**定義**: 自社のリソース(資金力や技術力など)に関連する事項を「強み」と「弱み」、外部の環境から受ける影響を「機会」と「脅威」にそれぞれ分類して分析する手法
...
**定義**: プロジェクトチームが実行すべき作業を、成果物を主体に階層的に要素分解したもの

**補足**: 最下層に位置する個々の部分をワークパッケージと呼ぶ

### CMMI

**正式名称**: Capability Maturity Model Integration

**別名**: 統合能力成熟度モデル

**定義**: 組織におけるプロセス改善をガイドするモデル
...
**定義**: プロジェクトの開始、組織編成と準備、作業の遂行、プロジェクトの完了などの各プロジェクト・フェーズの終了時に実施するレビュー
...
**定義**: 企業内のプロジェクトを横断的に支援する専門部署

**より詳細な定義**: 企業内で並行して実施されている個々のプロジェクトのマネジメント業務の支援、プロジェクトマネージャのサポート、部門間の調整などプロジェクトが円滑に実施されるように支援を行う専門の部署

**出題問題**: H28春午前問51

### ファストトラッキング技法

**定義**: 当初の計画では順番に行う予定だったアクティビティを並行して実施することによってスケジュール短縮を図る手法
...
**定義**: 自社のITサービスや管理の特性に応じた体系的な仕組み

### 是正措置

**定義**: 検知した不具合などの原因を除去・再発防止を防ぐ処置

### 継続的改善

**定義**: パフォーマンス向上のために繰り返し実施される活動

### JIQ Q 20000-1:2020

**定義**: サービスマネジメントシステムについて、実施・維持・改善するために、組織に対する要求事項を示したもの

### 変更管理

**定義**: ITサービスを構成する要素(ハードウェアやソフトウェア)に対する全ての変更を管理するプロセス
...
**正式名称**: Change Advisory Board(変更諮問委員会)

**特徴**: CABのメンバーは固定ではなく、変更内容に応じて変更される

### RFC

**正式名称**: Request For Change

**定義**: サービス・サーボスコンポーネントに対する変更の提案
...
**定義**: 監査人が監査対象となった母集団全体に関する結論を導き出すための合理的な基礎を得るため、母集団内のすべてのサンプリング単位に抽出の機会が与えられるような方法

**使用されるタイミング**: 母集団内の100%未満の項目に監査手続を適用する場合

### 監査サンプリングにおける許容逸脱率

**定義**: 受け入れることができる所定の内部統制からの逸脱率であり，監査人がサンプルの件数を決めるときに用いられる指標
